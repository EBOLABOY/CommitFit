...
authRoutes.post('/register', registerRateLimit, async (c) => {
  ...
  await c.env.DB.prepare(
    ...
    .bind(id, normalizedEmail, password_hash, normalizedNickname || null)
    .run();
  ...
  return c.json({
    success: true,
    data: { token, user: { id, email: normalizedEmail, nickname: normalizedNickname || null } },
  });
});
...
authRoutes.post('/login', loginRateLimit, async (c) => {
  ...
  const user = await c.env.DB.prepare(
    ...
    .bind(normalizedEmail)
    .first<{ id: string; email: string; password_hash: string; nickname: string | null }>();
...
authRoutes.get('/me', authMiddleware, async (c) => {
  ...
  const user = await c.env.DB.prepare('SELECT id, email, nickname FROM users WHERE id = ?')
    .bind(userId)
    .first<{ id: string; email: string; nickname: string | null }>();
