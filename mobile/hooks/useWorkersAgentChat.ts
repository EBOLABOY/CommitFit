...
type UseWorkersAgentChatOptions = {
  enabled?: boolean;
};

export function useWorkersAgentChat(sessionId = 'default', options?: UseWorkersAgentChatOptions) {
  ...
  }, [enabled, sessionId, updateLifecycleState]);
        ...
        if (WRITEBACK_TOOL_NAMES.has(toolName)) {
          ...
          if (isPlainObject(output)) {
            ...
            if (output.success === false) {
              ...
              const requestMeta = isPlainObject(output.request_meta)
                ? (output.request_meta as WritebackRequestMeta)
                : undefined;
  ...
  const sendMessageInternal = useCallback((text: string, imageDataUri: string | undefined, options: SendMessageOptions) => {
    if (!enabled) {
      setError('当前为自定义直连模式，Workers 通道未启用');
      return false;
    }
    const ws = wsRef.current;
    ...
    ws.send(JSON.stringify({
      ...
      init: {
        ...
        body: JSON.stringify({
          ...
          client_request_at: requestMeta.client_request_at,
          client_timezone: requestMeta.client_timezone,
          client_local_date: requestMeta.client_local_date,
          client_utc_offset_minutes: requestMeta.client_utc_offset_minutes,
        }),
      },
    }));
